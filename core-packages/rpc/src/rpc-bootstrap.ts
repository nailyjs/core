...
export class RpcBootstrap<Adapter> extends BackendBootstrap {
  ...
  async run(port: number, callback?: () => any): Promise<this> {
    const pluginContainer: RpcServerPlugin[] = Object.values(this.pluginContainer)

    // Run the beforeRun method of all rpc plugins.
    await Promise.all(pluginContainer.map(async plugin => await plugin.beforeRun?.(this)))
    ...
    await Promise.all(pluginContainer.map(async plugin => await plugin.beforeListen?.(this)))
    await adapter.listen(port, callback)
    // Run the afterListen method of all rpc plugins.
    await Promise.all(pluginContainer.map(async plugin => await plugin.afterListen?.(this)))
    return this
  }
   ...
   *
   * @param {RpcServerPlugin | RpcServerPlugin[]} rpcPlugin The rpc plugin.
   * @return {this}
   * @memberof RpcBootstrap
   */
  async use(rpcPlugin: RpcServerPlugin | RpcServerPlugin[]): Promise<this> {
    return super.use(rpcPlugin)
  }
}
